...
Beyond primary key indexing, three root causes consistently degrade Django Admin at scale.

**1. Unrestricted ForeignKey widget rendering.** When a ModelAdmin includes ForeignKey fields in its form, Django renders a `<select>` dropdown by default, which loads every row from the related table into memory to build the option list. At 500k+ related records, this single widget can consume hundreds of megabytes and block the response for seconds. Investigation: open Django Debug Toolbar on the add/change page and look for a query returning an absurdly large result set against the related table. The fix is switching to `autocomplete_fields` (which renders a Select2 widget backed by an AJAX search endpoint) or `raw_id_fields` (which renders a plain integer input with a lookup popup). Both eliminate the full-table load entirely — the database only ever executes a filtered, indexed query against the related table when the user actually types a search term.

**2. N+1 queries in `list_display`.** Any callable or dotted-path accessor in `list_display` that traverses a ForeignKey — for example, `order.customer.name` — fires a separate `SELECT` per row on the changelist page. With `list_per_page` at its default of 100, that is 100 additional queries per page load, and it gets worse if multiple columns do this. Investigation: enable Silk or Debug Toolbar on the changelist and look for repeated identical query shapes differing only by bound parameter. The fix is setting `list_select_related` to a list of the specific ForeignKey paths used in `list_display`. This forces Django's changelist queryset to emit a single SQL `JOIN`, collapsing N+1 into one round-trip. Avoid setting `list_select_related = True` blindly — it joins every FK on the model, which on a wide table with many relations produces an unnecessarily expensive query plan.

**3. Full-table `COUNT(*)` on every changelist load.** Django Admin calls `queryset.count()` to display "X results" at the top of the page, and PostgreSQL must perform a sequential scan or full index scan to compute an exact count on large tables because MVCC prevents it from caching row counts. Investigation: check Silk for a slow `SELECT COUNT(*) FROM ...` with no `WHERE` clause. Setting `show_full_result_count = False` on the ModelAdmin eliminates the unfiltered count query entirely. Additionally, reducing `list_per_page` to 25 or 50 limits the size of the data query itself and makes the paginator's offset-based fetches cheaper, since PostgreSQL still has to skip rows up to the requested offset before returning results.

### Question B — Pagination Trade-offs

Offset pagination (`LimitOffsetPagination`, `PageNumberPagination` in DRF) maps directly to SQL `LIMIT N OFFSET M`. It is simple to implement, supports jump-to-page navigation, and works naturally for admin dashboards and tabular UIs where users expect page numbers. The problem emerges at depth: `OFFSET 50000` forces PostgreSQL to fetch and discard 50,000 rows before returning the requested slice, even if every column involved is fully indexed. The scan cost grows linearly with page depth. Worse, if rows are inserted or deleted between paginated requests, the offset window shifts — producing duplicate or missing records across pages. For a dataset that changes infrequently and where users rarely paginate past page 10, this is an acceptable trade-off and the simpler choice.

Cursor pagination (`CursorPagination` in DRF) uses keyset pagination internally: instead of an offset, it encodes the last seen value of the ordering column into an opaque cursor token. The next page's query becomes `WHERE created_at < %s ORDER BY created_at DESC LIMIT N`, which PostgreSQL resolves via a simple index range scan regardless of how deep into the dataset the client has navigated — page 1 and page 500 have identical cost. It is also immune to row-shift problems: because the cursor anchors to a specific value rather than a positional offset, inserts and deletes between requests cannot cause duplicates or gaps. This makes it the correct choice for infinite-scroll feeds, mobile APIs, and any high-throughput endpoint where clients paginate deeply or data mutates frequently.

The trade-off is real, though. Cursor pagination requires a stable, unique ordering column (typically a timestamp plus a tiebreaker like `id`). It cannot support jump-to-page — the client must traverse sequentially. For internal tools or reports where users need to jump to page 47 or sort by arbitrary columns, offset pagination remains the practical choice despite its depth penalty. In production, I default to cursor for public-facing APIs and offset for back-office interfaces, choosing based on access pattern rather than dogma.
